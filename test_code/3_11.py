...
class TankMotion:
    ...
    def compute_motion_params(self, lat, lon, yaw, target_lat, target_lon, reverse=False):
        ...
        distance = np.sqrt(delta_lat**2 + delta_lon**2) * 6371000
...
class BasePlateCommand:
    ...
    def move_straight(self, distance):
        control = ht_control()
        control.mode = 1
        control.x = self.V if distance >= 0 else -self.V
        control.y = 0
        control.stop = 0
        duration = abs(distance) / (self.V / 1000)
        rospy.loginfo(f"{'倒退' if distance < 0 else '前进'}中... 距离: {abs(distance):.3f} m")
        for _ in range(int(duration)):
            self.pub.publish(control)
            rospy.sleep(1)
        rospy.sleep(duration % 1)
...
class AgrobotCommander:
    ...
    def stop_callback(self, msg):
        if msg.stop:
            rospy.logwarn("收到 Agrobot 停止指令！记录当前坐标...")
    ...
    def return_to_home(self):
        ...
        home_point = self.start_position
