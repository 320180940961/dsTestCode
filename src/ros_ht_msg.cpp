...
#include "ros_ht_msg/Lift_control.h"  //自定义消息类型
#include "ros_ht_msg/Lift_state.h"  //自定义消息类型
...
/**************************************底盘控制*******************************************/
void SendSpeedToHt(u8 mode,s16 x,int y,u8 stop)
{
	//需要发送的帧，结构体设置
	VCI_CAN_OBJ send[1];
...
void cmd_velCallback1(const geometry_msgs::Twist &twist_aux)
{
  SendSpeedToHt(2,twist_aux.linear.x*1000.0,twist_aux.angular.z*1000.0,0);
...
{
	SendSpeedToHt(ht_control_msg->mode,ht_control_msg->x,ht_control_msg->y,ht_control_msg->stop);
}
/**********************************END***********************************************/
...
static void cmd_LiftCallback(const ros_ht_msg::Lift_control::ConstPtr &lift_control_msg)//速度控制回调
{
...
int main(int argc, char **argv)
{
	...
	ros_ht_msg::Lift_state  lift_msg;
	ros_ht_msg::Lift_control lift_control_msg;
	printf(">>this is ht_hello !\r\n");//指示程序已运行
	...
	ros::Subscriber lift_control_sub = n.subscribe<ros_ht_msg::Lift_control>("/Lift_Control", 100, cmd_LiftCallback);//升降台速度回调
	...
	ros::Publisher lift_motion_pub      = n.advertise<ros_ht_msg::Lift_state>("/Lift_Motion", 100);//运动状态消息发布
	...
	while (ros::ok())
	{
		...
		if(re_size > 0)
		{
			for(int i=0;i< re_size;i++)
			{
				...
				{
					ht_motion_msg.Vx = rev[i].Data[2]|(rev[i].Data[3]<<8);
					temp_Vz = rev[i].Data[4]|(rev[i].Data[5]<<8);
					ht_motion_msg.Vz = (float)temp_Vz/1000.0f;
					ht_motion_msg.power = rev[i].Data[6]|(rev[i].Data[7]<<8);
				}
				else if((rev[i].ID == 0x50)&&(rev[i].Data[0] == 02) && (rev[i].Data[1] == 02))//接收底盘状态
				{
					ht_motion_msg.robot_data = rev[i].Data[2];
					ht_motion_msg.robot_contr_mode = rev[i].Data[3];
					ht_motion_msg.temp_l = rev[i].Data[4]|(rev[i].Data[5]<<8);
				}
				else if(rev[i].ID == 0x55)//驱动器状态反馈
				{
					ht_motion_msg.driver_err_L = rev[i].Data[0]|(rev[i].Data[1]<<8);
					ht_motion_msg.driver_err_R = rev[i].Data[2]|(rev[i].Data[3]<<8);
					ht_motion_msg.Current_L = rev[i].Data[4]|(rev[i].Data[5]<<8);
					ht_motion_msg.Current_R = rev[i].Data[6]|(rev[i].Data[7]<<8);
				}
				else if(rev[i].ID == 0x56)//驱动电机状态反馈
				{
					ht_motion_msg.driver_init_L = rev[i].Data[0]|(rev[i].Data[1]<<8);
					ht_motion_msg.driver_init_R = rev[i].Data[2]|(rev[i].Data[3]<<8);
					ht_motion_msg.Current_max_L = rev[i].Data[4]|(rev[i].Data[5]<<8);
					ht_motion_msg.Current_max_R = rev[i].Data[6]|(rev[i].Data[7]<<8);
				}
